...
export ZDOTFILES_CACHE_DIR="${ZDOTFILES_CACHE_DIR:-${CACHE_DIR}/zdotfiles}"
...
# P10K_INSTANT_PROMPT
# Must run before anything that produces console output (i.e. before Antidote).
# The cache file is generated by Powerlevel10k; harmlessly skipped when absent.
if [[ $ZSH_PROMPT == p10k && -r "${CACHE_DIR}/p10k-instant-prompt-${(%):-%n}.zsh" ]]; then
  builtin source "${CACHE_DIR}/p10k-instant-prompt-${(%):-%n}.zsh"
fi
# P10K_INSTANT_PROMPT end
